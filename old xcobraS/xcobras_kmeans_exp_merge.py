...
class XCOBRAS_kmeans(COBRAS_kmeans):
    def __init__(self, 
        ...
        self.arret_gt = False
        self.split_one_versus_all = split_one_versus_all
        self.merge_one_versus_all = merge_one_versus_all
    ...
    def determine_split_level(self, superinstance, clustering_to_store, indices_cluster_to_split = None):
        ...
        while not must_link_found and len(self.ml) + len(self.cl) < self.max_questions:
            ...
            y_hat = np.array([0]*len(self.data)) 
            explanations = [None, None]

            # # if self.xquerier == False:
            # #     explanations[0] = self.data[pt1]
            # #     explanations[1] = self.data[pt2]
            
            # if self.split_one_versus_all:
            #     # convert points of the same cluster to 1, others to 0
            #     mask = indices_cluster_to_split
            #     y_hat[mask] = 1 # label 1 for all the indices of that super_instance 
            #                     # label 0 for all the other data

            # else:
            #     # 2vsALL TODO ?
            #     # détecter cluster c de si
            #     # récupérer tous les super_instances de c
            #     # 
            #     ...



            # if len(set(y_hat)) > 1 and self.xquerier:
            #     # meaning number of class > 1
            #     print(f"Split:\tExplications!!!!!! | set(y_hat)={set(y_hat)} | size Class0:{sum(y_hat==0)}|size Class1:{sum(y_hat==1)}")
            #     self.nbr_expplications_used+=1
            ...
            # else: 
            #     # ground truth
            #     print(f"Split:\tVérité terrain!!!!!! | set(y_hat)={set(y_hat)} | size Class0:{sum(y_hat==0)}|size Class1:{sum(y_hat==1)}")
            #     self.nbr_data_used += 1
            #     # explanations[0] = self.data[pt1]
            #     # explanations[1] = self.data[pt1]
    ...
    def merge_containing_clusters(self, clustering_to_store):
        ...
        while merged and len(self.ml) + len(self.cl) < self.max_questions:
            ...
            for x, y in cluster_pairs:
                ...
                #### YACINE ####
                # 1. Cas1: X et Y concidéré comme 1 et les autres 0
                ...
                if self.merge_one_versus_all:
                    # convert points of the same cluster to 1, others to 0
        
                    mask1 = x.get_all_points()
                    mask0 = y.get_all_points()
                    y_hat = np.array([0]*len(self.data)) 
                    y_hat[mask1]  = 1 # label 1 for X
                    y_hat[mask0]  = 0 # label 0 for Y 

                    merging_data = np.array([self.data[i] for i in mask1 + mask0])

                    merging_y_hat = np.array([y_hat[i] for i in mask1 + mask0])
                
                if len(set(merging_y_hat)) > 1 and self.xquerier:
                    # meaning number of class > 1
                    print(f"Merge:\tExplications!!!!!! | set(merging_y_hat)={set(merging_y_hat)} | size Class0:{sum(merging_y_hat==0)}|size Class1:{sum(merging_y_hat==1)}")
                    self.nbr_expplications_used+=1
                    self.model_explainer.fit(merging_data, merging_y_hat)
                    shap_values = self.model_explainer.explain(np.array([self.data[pt1],self.data[pt2]]), feature_names=self.feature_names)
                    explanations[0] = shap_values[0]
                    explanations[1] = shap_values[1]
                else: 
                    # ground truth
                    print(f"Merge:\tVérité terrain!!!!!!| set(y_hat)={set(y_hat)} | size Class0:{sum(y_hat==0)}|size Class1:{sum(y_hat==1)}")
                    self.nbr_data_used += 1
                    # explanations[0] = self.data[pt1]
                    # explanations[1] = self.data[pt1]
                # #############################
